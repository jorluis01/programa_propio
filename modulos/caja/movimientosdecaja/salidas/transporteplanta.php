...
<div class="tipobodega">
    <p style="text-align: center"><h1 style="text-align: center">TRANSPORTE A PLANTA </h1> </p>
      ...
      <tbody>
        ...
        $dato=  mysqli_query($miconexion,$sql1);
         while($rs=mysqli_fetch_array($dato)){ 
            ...
            $sql="select SUM(valortransporte) as valor from transportematerial where idproveedortransporte=".$idcliente;
            
            $dato1=  mysqli_query($miconexion,$sql);
         while($rs2=mysqli_fetch_array($dato1)){
             $valor=$rs2['valor'];
         }
          $sql="select SUM(valor) as valor from movimientoaja where idclienteproveedor='".$idcliente."' and tipomovimiento='7'" ;
           
            $dato1=  mysqli_query($miconexion,$sql);
         while($rs2=mysqli_fetch_array($dato1)){
             $valorresta=$rs2['valor'];
         }
          $sql="select SUM(valortransporte) as valor from costoscompra where idproveedortransporteplanta='".$idcliente."' " ;
           
            $dato1=  mysqli_query($miconexion,$sql);
         while($rs2=mysqli_fetch_array($dato1)){
             $valor2=$rs2['valor'];
         }
         $valortotal=$valor-$valorresta+$valor2;
        echo "
             <tr>
